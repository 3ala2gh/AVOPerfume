...
export default function DiscountManagementSection({ products, isLoading, isSaving, onSave }: Props) {
  ...
  return (
    <AdminCollapsibleSection
      title="Discounts"
      description="Apply a campaign to all perfumes or hand-pick the products you want on sale."
    >
      <form onSubmit={submit} className="space-y-4">
        <label className="block text-sm">
          <span className="mb-1 block">Discount percentage</span>
          <Input type="number" min="0" max="100" step="0.01" required value={discount} onChange={(e) => setDiscount(e.target.value)} placeholder="25" />
        </label>
        ...
        {!applyToAll && <div className="space-y-2">
          ...
          <div className="max-h-52 overflow-y-auto rounded-lg border border-black/10 bg-white/80 p-2">
            {isLoading ? <p className="p-2 text-sm text-black/60">Loading...</p> : visible.map((product) => <label key={product.id} className="flex items-center gap-2 rounded-md p-2 text-sm hover:bg-black/5">
